...
// Per-slide image Fit/Position. Used by every slide-image render so users can:
//   - choose "fit" (whole image visible, letterboxed) vs the default "fill" (crop to frame)
//   - drag the focal point (objectPosition) so the crop or letterbox shows the right part
// Defaults preserve the original cover-crop behavior so stock-image slides are unchanged.
function slideImgStyle(slide) {
  var fit = slide && slide.imgFit === "fit" ? "contain" : "cover";
  var pos = slide && slide.imgPosition;
  var x = pos && typeof pos.x === "number" ? pos.x : 50;
  var y = pos && typeof pos.y === "number" ? pos.y : 50;
  return { objectFit: fit, objectPosition: x + "% " + y + "%" };
}

// Letterbox background when "fit" mode leaves blank area around the image. Defaults to black
// so light photos still pop; users can override via slide.imgLetterbox.
function slideImgLetterbox(slide) {
  if (!slide || slide.imgFit !== "fit") return null;
...
function ImgBg({ url, pal, children, darken, category, imgFilter, slideIndex, slide }) {
  ...
  var imgStyle = slideImgStyle(slide);
  var letterbox = slideImgLetterbox(slide);
  var bg = letterbox || pal.bg;
  return (
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden", background: bg }}>
      {url && <img src={url} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: imgStyle.objectFit, objectPosition: imgStyle.objectPosition, filter: filt }} onError={function(e) { e.target.style.display = "none"; }} />}
...
function S4Emigre({ slide, index, category, images }) {
  ...
  if (fmt === "comparison") {
    ...
    return (
      <ImgBg url={url} pal={p} category={category} slideIndex={index || 0} slide={slide} darken={"rgba(0,0,0," + compScrim + ")"}>
        {compLayout === 0 ? (
  ...
  if (fmt === "killer") {
    ...
    return (
      <ImgBg url={url} pal={p} category={category} slideIndex={index || 0} slide={slide} darken={"rgba(0,0,0," + killerScrim + ")"}>
        {killerLayout === 0 ? (
  ...
  if (fmt === "story") {
    ...
    return (
      <ImgBg url={url} pal={p} category={category} slideIndex={index || 0} slide={slide} darken={"rgba(0,0,0," + storyScrim + ")"}>
        {storyLayout === 0 ? (
  ...
  if (fmt === "versus") {
    ...
    return (
      <ImgBg url={url} pal={p} category={category} slideIndex={index || 0} slide={slide} darken={"rgba(0,0,0," + vsScrim + ")"}>
  ...
  return (
    <ImgBg url={url} pal={p} category={category} slideIndex={index || 0} slide={slide} darken={"rgba(0,0,0," + tlScrim + ")"}>
...
function S5Face({ slide, index, category, images }) {
  ...
  var s5Bgfill = (s5Mosaic && s5Mosaic.length >= 2)
    ...
    : url
      ? <img src={url} alt="" style={Object.assign({ width: "100%", height: "100%", filter: "saturate(0.85) brightness(0.75)" }, slideImgStyle(slide))} onError={function(e) { e.target.style.display = "none"; }} />
...
function SlideRenderer({ category, slideData, slideIndex, totalSlides, images, edition }) {
  ...
  if (category === "enterprise") {
    ...
    if (slideIndex === lastIdx) slide = <S7Blitz category={category} hashtags={slideData.hashtags || ""} images={images} index={slideIndex} slide={slideData} />;
...
function buildPrompt(catLabel, topic, editionSeed, picks, hasPersonImage, secondaryCatLabel, tertiaryCatLabel, secCount, terCount) {
  ...
  // Topic-intent detection — listicle/current/timely queries should NOT get an editorial pivot
  var topicLc = (topic || "").toLowerCase();
  var LITERAL_PATTERNS = /\b(currently|right now|this week|this month|today|tonight|now streaming|trending|best|top|worst|cheapest|highest|lowest|most popular|new|latest|upcoming|2026|2027|what'?s on|what'?s in|where to|how to|guide to|list of|recommend|reviewed)\b/;
  var isLiteralTopic = LITERAL_PATTERNS.test(topicLc);
  // Angle: -2 = Literal (suppressed), -1 = Random, 0+ = explicit pick
  var angleSuppressed = p.angle === -2 || isLiteralTopic;
  var freshness = "";
  if (!angleSuppressed) {
    var freshSeed = p.angle >= 0 && FRESHNESS_SEEDS[p.angle] ? FRESHNESS_SEEDS[p.angle] : pickFreshness(editionSeed || 0);
    freshness = typeof freshSeed === "string" ? freshSeed : freshSeed.prompt;
  }
...
export default function LoathrMediaGenerator() {
  ...
  var factCheckDeep = _cb(async function() {
    ...
    try {
      ...
      // PASS 1 — extract claims (no web_search, just structured extraction)
      var extractPrompt = "Extract every CHECKABLE FACTUAL CLAIM from these carousel slides as a structured list. A factual claim is any specific number, date, dollar amount, named person, named organization, named product, named event, direct quote, attribution, or specific assertion about reality. Skip opinion, narrative voice, framing, and stylistic phrasing — only checkable assertions.\n\nSlides:\n" + slideTexts + "\n\nRespond ONLY with JSON, no prose, no markdown:\n{\"claims\": [{\"slide\": N, \"field\": \"body|stat|heading|quote|highlight|etc\", \"claim\": \"the specific assertion as a self-contained sentence\", \"type\": \"number|date|quote|attribution|event|stat|generic\"}]}";
      var er = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-opus-4-7", max_tokens: 8000, messages: [{ role: "user", content: extractPrompt }] }) });
      var ed = await er.json();
      if (ed.error) { setFactCheckResult({ score: 0, summary: "Deep check (extract pass) failed: " + (ed.error.message || JSON.stringify(ed.error)), issues: [], _failed: true }); return; }
      var eText = (ed.content || []).filter(function(b) { return b.type === "text"; }).map(function(b) { return b.text; }).join("").replace(/```json|```/g, "").trim();
      var ejs = eText.indexOf("{"); var eje = eText.lastIndexOf("}");
      if (ejs >= 0 && eje > ejs) eText = eText.slice(ejs, eje + 1);
      var extracted; try { extracted = JSON.parse(eText); } catch (pe) { setFactCheckResult({ score: 0, summary: "Deep check extract pass returned invalid JSON. Try again.", issues: [], _failed: true }); return; }
      var claims = Array.isArray(extracted.claims) ? extracted.claims : [];
      if (claims.length === 0) { setFactCheckResult({ score: 10, summary: "Deep check found no checkable factual claims in this carousel.", issues: [] }); return; }
      ...
      var vText = (vd.content || []).filter(function(b) { return b.type === "text"; }).map(function(b) { return b.text.replace(/<cite[^>]*>/g, "").replace(/<\/cite>/g, ""); }).join("").replace(/```json|```/g, "").trim();
      var vjs = vText.indexOf("{"); var vje = vText.lastIndexOf("}");
      if (vjs >= 0 && vje > vjs) vText = vText.slice(vjs, vje + 1);
      if (!vText) { setFactCheckResult({ score: 0, summary: "Deep check verify pass returned no output. Try again.", issues: [], _failed: true }); return; }
          ...
          {showEditionSettings && <div style={{ marginBottom: 10, padding: 10, border: "0.5px solid " + (activeSegment === "enterprise" ? "#333" : activeSegment === "newsdesk" ? "#d8d6d0" : "var(--color-border-tertiary)"), background: activeSegment === "enterprise" ? "#111" : activeSegment === "newsdesk" ? "#fafaf7" : "#fafafa" }}>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                ...
                <button onClick={function() { setEditionPicks(function(p) { return Object.assign({}, p, { angle: -2 }); }); }}
                  title="No editorial angle — answer the topic literally. Best for current/listicle queries."
                  style={{ padding: "3px 8px", border: "0.5px solid #ddd", background: editionPicks.angle === -2 ? uiAccent + "22" : "transparent", color: editionPicks.angle === -2 ? uiAccent : "#999", cursor: "pointer", ...CP, fontSize: 6 }}>Literal</button>
              ...
              {editionPicks.angle === -2 && <div style={{ ...CP, fontSize: 5, color: "#999", marginTop: 2 }}>No angle. Answer the topic as written, no editorial pivot.</div>}
      ...
      {cur && <div style={{ marginBottom: 18, textAlign: "center" }}>
        ...
        {editMode && cur && (function() {
          ...
          return <div style={{ marginTop: 6, border: "0.5px solid " + uiAccent + "44", background: "#f8f8f8", padding: 8, borderRadius: 3 }}>
            ...
            {editSection === "layout" && <div>
              ...
              {/* Image Fit / Position — visible whenever this slide has an image */}
              {(function() {
                var imgEntry = images && images[currentSlide];
                if (!imgEntry || !imgEntry.url) return null;
                var fit = s.imgFit || "fill";
                var pos = s.imgPosition || { x: 50, y: 50 };
                var px = typeof pos.x === "number" ? pos.x : 50;
                var py = typeof pos.y === "number" ? pos.y : 50;
                var darkSeg = activeSegment === "enterprise";
                var labelStyle = { ...CP, fontSize: 5, color: darkSeg ? "#aaa" : "#888", marginBottom: 3 };
                var smallStyle = { ...CP, fontSize: 5, color: darkSeg ? "#999" : "#666", width: 28 };
                ...
                var isUpload = imgEntry.source === "Upload";
                return <div style={{ marginBottom: 6, padding: "6px 8px", border: "0.5px dashed " + (darkSeg ? "#444" : "#ddd"), borderRadius: 3 }}>
                  <div style={labelStyle}>IMAGE FIT{isUpload ? " · UPLOADED" : ""}</div>
                  <div style={{ display: "flex", gap: 4, marginBottom: 6 }}>
                    <button onClick={function() { updateSlideField(currentSlide, "imgFit", "fill"); }} style={pillStyle(fit === "fill")} title="Crop the image to fill the slide frame (default)">Fill</button>
                    <button onClick={function() { updateSlideField(currentSlide, "imgFit", "fit"); }} style={pillStyle(fit === "fit")} title="Show the whole image, letterboxed around it">Fit</button>
                  </div>
                  <div style={labelStyle}>FOCAL POINT</div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                    <span style={smallStyle}>X</span>
                    <input type="range" min="0" max="100" step="1" value={px} onChange={function(e) { updateSlideField(currentSlide, "imgPosition", { x: parseInt(e.target.value, 10), y: py }); }} style={{ flex: 1 }} />
                    <span style={Object.assign({}, smallStyle, { textAlign: "right" })}>{px}%</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4 }}>
                    <span style={smallStyle}>Y</span>
                    <input type="range" min="0" max="100" step="1" value={py} onChange={function(e) { updateSlideField(currentSlide, "imgPosition", { x: px, y: parseInt(e.target.value, 10) }); }} style={{ flex: 1 }} />
                    <span style={Object.assign({}, smallStyle, { textAlign: "right" })}>{py}%</span>
                  </div>
                  {fit === "fit" && <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4 }}>
                    <span style={Object.assign({}, smallStyle, { width: 56 })}>LETTERBOX</span>
                    <input type="color" value={s.imgLetterbox || "#000000"} onChange={function(e) { updateSlideField(currentSlide, "imgLetterbox", e.target.value); }} style={{ width: 30, height: 18, padding: 0, border: "0.5px solid " + (darkSeg ? "#444" : "#ddd"), cursor: "pointer", background: darkSeg ? "#000" : "#fff" }} />
                  </div>}
              ...
              {/* Enterprise role label picker — visible on content slides. Default labels are
                  generated by Claude (e.g. THE DATA, THE IMPACT) but were previously not
                  editable from the slide editor. Now: dropdown of presets + a custom text
                  field for any free-form label. */}
              {activeSegment === "enterprise" && isContent && (function() {
                var ENT_ROLES = [
                  "THE LANDSCAPE", "THE FORCE", "THE IMPACT", "THE WINNERS", "THE LOSERS",
                  "THE DATA", "THE PLAYBOOK", "THE FORECAST", "THE ORIGIN", "THE TURNING POINT",
                  "THE HOT TAKE", "THE HUMAN STORY", "THE EVIDENCE", "THE VOICE", "THE RIPPLE EFFECT",
                  "THE COUNTER", "THE DEEP CUT", "THE NOW",
                  "WHAT HAPPENED", "WHO'S AFFECTED", "THE NUMBERS", "WHAT TO DO"
                ];
                var curRole = String(s.role || "").toUpperCase();
                var isPreset = ENT_ROLES.indexOf(curRole) > -1;
                return <div style={{ marginBottom: 6 }}>
                  <div style={{ ...CP, fontSize: 5, color: "#888", marginBottom: 3 }}>ROLE LABEL</div>
                  <div style={{ display: "flex", gap: 2, flexWrap: "wrap", marginBottom: 4 }}>
                    {ENT_ROLES.map(function(r) {
                      var sel = curRole === r;
                      return <button key={r} onClick={function() { updateSlideField(currentSlide, "role", r); }}
                        style={{ padding: "2px 5px", border: "0.5px solid " + (sel ? "#fff" : "#444"), background: sel ? "#ffffff22" : "transparent", cursor: "pointer", ...CP, fontSize: 4, color: sel ? "#fff" : "#888", letterSpacing: "0.05em" }}>{r}</button>;
                    })}
                  </div>
                  <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    <span style={{ ...CP, fontSize: 5, color: "#666" }}>Custom:</span>
                    <input value={!isPreset ? (s.role || "") : ""} placeholder="Type any role label"
                      onChange={function(e) { updateSlideField(currentSlide, "role", e.target.value); }}
                      style={{ flex: 1, padding: "3px 6px", border: "0.5px solid #444", background: "#1a1a1a", color: "#eee", ...CP, fontSize: 6 }} />
                    {s.role && <button onClick={function() { updateSlideField(currentSlide, "role", ""); }}
                      style={{ padding: "2px 6px", border: "0.5px solid #444", background: "transparent", cursor: "pointer", ...CP, fontSize: 5, color: "#888" }}>Clear</button>}
                  </div>
                  {/* Playbook step tools — only when this slide is a THE PLAYBOOK slide.
                      EnterprisePlaybook renders slide.body parsed as numbered steps; this
                      adds a button to append a new step token so the user doesn't have to
                      manually type the next number. */}
                  {curRole === "THE PLAYBOOK" && <div style={{ marginTop: 6, paddingTop: 4, borderTop: "0.5px solid #333" }}>
                    <div style={{ ...CP, fontSize: 5, color: "#888", marginBottom: 3 }}>PLAYBOOK STEPS</div>
                    {(function() {
                      var body = s.body || "";
                      // Count existing numbered steps so the new one is N+1.
                      var matches = body.match(/(?:^|\n)\s*\d+[\.\)]\s/g) || [];
                      var next = matches.length + 1;
                      return <div style={{ display: "flex", gap: 4 }}>
                        <button onClick={function() {
                          var existing = (s.body || "").replace(/\s+$/, "");
                          var nl = existing ? "\n\n" : "";
                          updateSlideField(currentSlide, "body", existing + nl + next + ". ");
                        }}
                          style={{ padding: "3px 8px", border: "0.5px solid #fff", background: "#ffffff11", cursor: "pointer", ...CP, fontSize: 6, color: "#fff", letterSpacing: "0.05em" }}>{"+"} Add Step {next}</button>
                        {matches.length > 0 && <span style={{ ...CP, fontSize: 4, color: "#666", alignSelf: "center" }}>{matches.length} step{matches.length === 1 ? "" : "s"} in body</span>}
                      </div>;
                    })()}
                  </div>}
                </div>;
        ...
        {factCheckResult && factCheckResult.issues && factCheckResult.issues.length > 0 && <div style={{ marginTop: 6, border: "0.5px solid " + (factCheckResult.score >= 7 ? "#22c55e44" : "#ef444444"), background: "#f8f8f8", padding: 6, borderRadius: 3 }}>
          ...
          {factCheckResult.issues.map(function(issue, i) {
            ...
            var confColor = conf === "high" ? "#dc2626" : conf === "medium" ? "#f59e0b" : conf === "low" ? "#888" : null;
            var urls = Array.isArray(issue.verifiedAgainst) ? issue.verifiedAgainst.filter(function(u) { return typeof u === "string" && /^https?:/.test(u); }) : [];
            return (
              <div key={i} onClick={canJump ? function() { setCurrentSlide(issue.slide); } : null}
                ...
                <span style={{ color: "#ef4444", fontWeight: 700 }}>Slide {(typeof issue.slide === "number" ? issue.slide + 1 : "?")}{issue.field ? " \u00b7 " + issue.field : ""}:</span>
                {confColor && <span style={{ marginLeft: 3, padding: "0 4px", border: "0.5px solid " + confColor, color: confColor, ...CP, fontSize: 4, letterSpacing: "0.08em", textTransform: "uppercase" }}>{conf}</span>}
                {" " + issue.issue}
                {issue.fix && <span style={{ color: "#22c55e" }}> {"\u2192"} {issue.fix}</span>}
                {urls.length > 0 && <span style={{ marginLeft: 4, color: "#3b82f6" }}>{urls.map(function(u, ui) {
                  var host = u.replace(/^https?:\/\//, "").split("/")[0].replace(/^www\./, "");
